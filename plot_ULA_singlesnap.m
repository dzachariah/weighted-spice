...
for m = 1:MC_runs
    ...
    %Generate signals
    %---------------------------
    %Generate SOI (random phase)
    x_true = chol(P_true)' * exp(1i*2*pi*rand(K_true, T_snap)); %(randn(K_true,T_snap) + 1i*randn(K_true,T_snap))/sqrt(2);
...
%Compute mean
amp_spice_mean = mean( abs(x_hat_spice_lmvue), 2 );
amp_likes_mean = mean( abs(x_hat_likes_lmvue), 2 );
amp_slim_mean  = mean( abs(x_hat_slim_lmvue), 2 );
amp_iaa_mean   = mean( abs(x_hat_iaa_lmvue), 2 );
...
figure(1)
subplot(2,2,1)
semilogy( theta_set*(180/pi), amp_spice_mean, 'b-', 'LineWidth', 1.2 ), hold on, grid on
semilogy( theta_true*(180/pi), sqrt(P_diag_true), 'kx', 'LineWidth', 1.5 )
legend('SPICE','true'), xlabel('\theta [deg.]'), ylabel('|\hat{x}|')
axis([-90 90  1e-3 5])

%figure(2)
subplot(2,2,2)
semilogy( theta_set*(180/pi), amp_likes_mean, 'b-', 'LineWidth', 1.2 ), hold on, grid on
semilogy( theta_true*(180/pi), sqrt(P_diag_true), 'kx', 'LineWidth', 1.5 )
legend('LIKES','true'), xlabel('\theta [deg.]'), ylabel('|\hat{x}|')
axis([-90 90  1e-3 5])

%figure(3)
subplot(2,2,3)
semilogy( theta_set*(180/pi), amp_slim_mean, 'b-', 'LineWidth', 1.2 ), hold on, grid on
semilogy( theta_true*(180/pi), sqrt(P_diag_true), 'kx', 'LineWidth', 1.5 )
legend('SLIM','true'), xlabel('\theta [deg.]'), ylabel('|\hat{x}|')
axis([-90 90  1e-3 5])

%figure(4)
subplot(2,2,4)
semilogy( theta_set*(180/pi), amp_iaa_mean, 'b-', 'LineWidth', 1.2 ), hold on, grid on
semilogy( theta_true*(180/pi), sqrt(P_diag_true), 'kx', 'LineWidth', 1.5 )
legend('IAA','true'), xlabel('\theta [deg.]'), ylabel('|\hat{x}|')
axis([-90 90  1e-3 5])
